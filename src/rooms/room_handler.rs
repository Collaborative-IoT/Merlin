...
use crate::communication::communication_types::{UserRemovedFromRoom,BasicResponse};
use crate::communication::data_capturer::CaptureResult;
use crate::rabbitmq::rabbit;
use warp::ws::Message;
use crate::state::state_helpers;
...
pub async fn block_user_from_room(
    ...
    execution_handler:&Arc<Mutex<ExecutionHandler>>
        ...
        if state.rooms.contains_key(&room_id) && state.rooms.get(&room_id).unwrap().user_ids.contains(&user_id.to_string()){
            ...
            if owner_gather.0 == false && owner_gather.1 == requester_id{
                ...
                handle_user_block_capture_result(capture_result, requester_id, user_id.clone(),  &mut state,room_id).await;
                return;
            }
        }
        send_error_to_requester_channel(user_id,requester_id,&mut state,"issue_blocking_user".to_string());
...
pub async fn handle_user_block_capture_result(
    ...
    if capture_result.encountered_error == false{
        ...
        server_state.rooms.get_mut(&room_id).unwrap().user_ids.remove(&user_id.to_string());
    }
    send_error_to_requester_channel(user_id,requester_id,server_state,"issue_blocking_user".to_string());
}

fn send_error_to_requester_channel(user_id:i32, requester_id:i32, server_state:&mut ServerState, op_code:String){
    let response = BasicResponse{
        response_op_code:op_code,
        response_containing_data:user_id.to_string()
    };
    //TODO:handle error
    server_state.peer_map.get(&requester_id).unwrap().send(Message::text(serde_json::to_string(&response).unwrap()));
}