...
pub async fn test_gathering_user_preview(
...
pub async fn test_gathering_user_by_github_discord_id(execution_handler: &mut ExecutionHandler) {
    let discord_id = "2".to_owned();
    let github_id = "1".to_owned();
    let select_user_result = execution_handler
...
pub async fn test_updating_user_avatar(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    let result = execution_handler
        .update_user_avatar(new_avatar_url.to_owned(), &user_id)
        .await;
...
pub async fn test_updating_ban_status_of_user(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_ban_status_of_user(new_banned, new_banned_reason, &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check after update
...
pub async fn test_updating_user_display_name(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_display_name(new_display_name.to_owned(), &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check after update
...
pub async fn test_updating_user_bio(execution_handler: &mut ExecutionHandler, user_id: i32) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_user_bio(new_bio.to_owned(), &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check_after_update
    test_user_column_after_update(11 as usize, new_bio.to_owned(), &user_id, execution_handler)
        .await;
    return user_id;
}

pub async fn test_updating_last_online(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
...
pub async fn test_updating_github_access_token(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_github_access_token(new_access_token, &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check_after_update
...
pub async fn test_updating_discord_access_token(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_discord_access_token(new_discord_access_token, &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check after update
...
pub async fn test_update_contributions(
    execution_handler: &mut ExecutionHandler,
    user_id: i32,
) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_contributions(&new_contributions, &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check after update
...
pub async fn test_update_user_name(execution_handler: &mut ExecutionHandler, user_id: i32) -> i32 {
    ...
    //update
    let result = execution_handler
        .update_user_name(new_user_name, &user_id)
        .await;
    let num_of_rows_updated = result.unwrap();
    assert_eq!(num_of_rows_updated, 1);
    //check after
...
pub async fn test_updating_entire_user(execution_handler: &mut ExecutionHandler) -> i32 {
    ...
    let select_row_result_second = execution_handler
        .select_user_by_id(&different_user.id)
        .await;
    let selected_rows_second = select_row_result_second.unwrap();
...
async fn test_user_column_after_update<
    T: std::fmt::Debug + std::cmp::PartialEq + for<'a> tokio_postgres::types::FromSql<'a>,
>(
    col_num: usize,
    value_expected: T,
    user_id: &i32,
