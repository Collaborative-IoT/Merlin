...
pub fn web_rtc_request_is_valid(server_state:&ServerState, request_data:serde_json::Value, requester_id:&i32)->bool{
    ...
    if request_data["peerId"] == serde_json::Value::Null || request_data["roomId"] == serde_json::Value::Null{
        return false;
    }
    ...
    if &user_id_from_request == requester_id && server_state.rooms.contains_key(&room_id_from_request) && server_state.rooms.get(&room_id_from_request).unwrap().user_ids.contains(&user_id_from_request){
        return true;
    }
